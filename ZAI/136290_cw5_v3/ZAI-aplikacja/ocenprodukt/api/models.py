...
class Opinia(models.Model):
    ...
    def __str__(self):
        return str(self.produkt) + ', autor: ' + self.autor

class Sklep(models.Model):
    nazwa_sklepu = models.CharField(max_length=30)
    adres_sklepu = models.URLField()
    produkty = models.ManyToManyField(Produkt, related_name='produkty', null=True, blank=True)
    nip = models.PositiveIntegerField(validators=[MaxValueValidator(9999999999)])
    telefon = PhoneNumberField(null=False, blank=False, unique=True)
    fax = PhoneNumberField(null=False, blank=False, unique=False)
